...
class StrategyConfig:
    ...
    max_gap_z: float = 3.0
    max_impulse_atr: float = 3.5
    max_extension_from_vwap_atr: float = 2.0
    ...
    w_retracement: float = 0.24
    w_relative_strength: float = 0.18
    w_volume: float = 0.10
    w_vwap: float = 0.10
    ...
    def validate(self) -> None:
        weights = (self.w_impulse, self.w_retracement, self.w_relative_strength,
                   self.w_volume, self.w_vwap, self.w_structure, self.w_volatility)
        if abs(sum(weights) - 1.0) > 1e-9:
            raise ValueError("ranking weights must sum to exactly 1.0")
        if self.min_retracement_depth >= self.max_retracement_depth:
            raise ValueError("invalid retracement interval")
